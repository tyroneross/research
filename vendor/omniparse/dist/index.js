  ...
  "node_modules/xlsx/xlsx.mjs"() {
    ...
    CFB = /* @__PURE__ */ (function _CFB() {
      ...
      function read_file(filename2, options) {
        get_fs();
        return parse2(fs5.readFileSync(filename2), options);
      }
...
var fs, path, MAX_FILE_SIZE, PIPE_RE, NEWLINE_RE;
...
var fs2, path2, zlib2, import_sax, MAX_FILE_SIZE2, SLIDE_PATTERN, NOTES_PATTERN, CHART_PATTERN, DIAGRAM_PATTERN, AT_REGEX, AP_SPLIT, CV_REGEX, XML_ENTITIES2, ENTITY_RE;
var init_pptx_parser_fast = __esm({
  "src/parsers/pptx-parser-fast.ts"() {
    fs2 = __toESM(require("fs"));
    path2 = __toESM(require("path"));
    zlib2 = __toESM(require("zlib"));
...
var EXCEL_EXTENSIONS = /* @__PURE__ */ new Set([".xlsx", ".xls", ".csv", ".tsv", ".ods", ".xlsb"]);
var PPTX_EXTENSIONS = /* @__PURE__ */ new Set([".pptx"]);
...
function detectInputType(input) {
  try {
    ...
    if (fs4.existsSync(resolved)) {
      ...
      if (stat.isFile()) {
        const ext2 = path4.extname(resolved).toLowerCase();
        if (EXCEL_EXTENSIONS.has(ext2)) return "excel";
        if (PPTX_EXTENSIONS.has(ext2)) return "pptx";
        if (PYTHON_EXTENSIONS.has(ext2)) return "python";
        if (PDF_EXTENSIONS.has(ext2)) return "pdf";
        return "unsupported";
      }
    }
...
async function parse(input, options = {}) {
  ...
  switch (inputType) {
    ...
    case "unsupported":
      throw new Error(
        `Unsupported input: "${input.substring(0, 100)}". Supported: .xlsx, .xls, .csv, .pptx, .py, .pdf, or directories.`
      );
  }
}
async function parseMultiple(inputs, options = {}) {
  const concurrency = options.concurrency ?? 4;
  const { default: pLimit2 } = await Promise.resolve().then(() => (init_p_limit(), p_limit_exports));
  const limit = pLimit2(concurrency);
  let completed = 0;
  const promises = inputs.map(
    (input, idx) => limit(async () => {
      try {
...
async function parseExcel(filePath, options) {
  ...
  const result = parseExcelFile2(filePath, {
  ...
  const metadata = {
    ...
    sheets: result.sheets.map((s) => ({
      ...
      rows: options.includeSheetRows ? s.rawData.slice(1).map((row) => row.map((cell) => String(cell ?? ""))) : void 0
    })),
...
async function parsePython(filePath) {
...
${text || "*No extractable text found. PDF may be image-based.*"}`,
    ...
    errors: text.length === 0 ? ["No extractable text found. PDF may be image-based or encrypted."] : void 0
  };
}
async function parseDirectory(dirPath, options) {
  const supportedExts = /* @__PURE__ */ new Set([
    ...EXCEL_EXTENSIONS,
    ...PPTX_EXTENSIONS,
    ...PYTHON_EXTENSIONS,
    ...PDF_EXTENSIONS
...
function findFiles(dirPath, recursive, extensions) {
  const files = [];
...
function createErrorResult(input, inputType, error, startTime) {
  return {
    input,
    inputType,
    fileName: path4.basename(input),
    markdown: `# Error
...
function extractBtEtText(content, parts) {
  ...
  while ((btMatch = btEtPattern.exec(content)) !== null) {
    ...
    while ((tjArrMatch = tjArrPattern.exec(block)) !== null) {
      ...
      while ((strMatch = strPattern.exec(tjArrMatch[1])) !== null) {
        strs2.push(decodePdfStr(strMatch[1]));
      }
      if (strs2.length > 0) parts.push(strs2.join(""));
    }
  }
}
...
init_excel_parser_fast();
init_excel_parser_rich();
init_pptx_parser_fast();
init_python_parser();
