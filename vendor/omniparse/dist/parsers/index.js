...
function parse_sheet_legacy_drawing(sheet, type, zip, path4, idx, opts, wb, comments) {
  if (!sheet || !sheet["!legdrawel"]) return;
  var dfile = resolve_path(sheet["!legdrawel"].Target, path4);
...
function safe_parse_sheet(zip, path4, relsPath, sheet, idx, sheetRels, sheets, stype, opts, wb, themes, styles) {
  try {
    ...
    parse_sheet_legacy_drawing(_ws, stype, zip, path4, idx, opts, wb, comments);
  } catch (e) {
    if (opts.WTF) throw e;
  }
}
...
function parse_zip(zip, opts) {
  ...
  wsloop: for (i = 0; i != props.Worksheets; ++i) {
    ...
    if (wbrels && wbrels[i]) {
      ...
      if (!safegetzipfile(zip, path4)) path4 = wbrelsfile.replace(/_rels\/[\S\s]*$/, "") + wbrels[i][1];
      stype = wbrels[i][2];
    } else {
...
var qreg = /"/g;
function make_csv_row(sheet, r, R, cols, fs4, rs, FS, w, o) {
  var isempty = true;
...
function sheet_to_csv(sheet, opts) {
  ...
  for (var R = r.s.r; R <= r.e.r; ++R) {
    if ((rowinfo[R] || {}).hidden) continue;
    row = make_csv_row(sheet, r, R, cols, fs4, rs, FS, w, o);
    if (row == null) {
      continue;
    }
...
var import_sax = __toESM(require_sax());
var MAX_FILE_SIZE2 = 100 * 1024 * 1024;
var SLIDE_PATTERN = /^ppt\/slides\/slide(\d+)\.xml$/i;
var NOTES_PATTERN = /^ppt\/notesSlides\/notesSlide(\d+)\.xml$/i;
var CHART_PATTERN = /^ppt\/charts\/chart\d+\.xml$/i;
var DIAGRAM_PATTERN = /^ppt\/diagrams\/data\d+\.xml$/i;
var AT_REGEX = /<a:t>([^<]*)<\/a:t>/g;
var AP_SPLIT = /<\/a:p>/g;
var CV_REGEX = /<c:v>([^<]*)<\/c:v>/g;
var XML_ENTITIES2 = {
  "&amp;": "&",
  "&lt;": "<",
  "&gt;": ">",
...
var ENTITY_RE = /&(?:amp|lt|gt|apos|quot);/g;
